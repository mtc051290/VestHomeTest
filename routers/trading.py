...
async def buy_shares( buy_shares  : BuyShares,
    ...
    response = requests.get( url_quote_info, params=my_params, headers=hack_headers, timeout=5 )
    ...
    # Create stock in database if it doesn't exist
    stock_id = trading_functions.create_nasdaq_stock_if_not_exists(data, db)
    
    # Fetch, then create or add shares list
    ...
    try:
        purchase_summary = PurchaseSummary(
            date_time_purchase = datetime.now(time_zone).strftime("%Y-%m-%d %H:%M:%f"),
            company=data['companyName'],
            symbol=data['symbol'],
            total_purchased_shares=buy_shares.quantity,
            unitary_price= data['primaryData']['lastSalePrice'][1:],
            total_price=(float(buy_shares.quantity) * float(data['primaryData']['lastSalePrice'][1:])),
            pending=has_pending,
            message=msg
        )
...
async def sell_shares( sell_shares  : SellShares,
    ...
    # Apply FIFO, save the date, save sold price and check if pending
    count = 0
    ac_difference = 0
    ac_bought_price = 0
    for index, item in enumerate(shares):
        if count >= available_shares:
            break
        if shares[index]['sold'] == shares[index]['pending'] == False:
            ...
            shares[index]['sold_date'] = datetime.now(time_zone).strftime("%Y-%m-%d %H:%M:%f")
    ...
    try:
        sell_summary = SaleSummary(
            date_time_sold = datetime.now(time_zone).strftime("%Y-%m-%d %H:%M:%f"),
            company = data['companyName'],
            symbol = data['symbol'],
            total_sold_shares = sell_shares.quantity,
            unitary_sold_price = data['primaryData']['lastSalePrice'][1:],
            total_sold_price =(float(sell_shares.quantity) * float(data['primaryData']['lastSalePrice'][1:])),
            total_bought_price = 0.00,
            difference = 0.00,
            profit_loss = 0.00,
            pending = pending,
            message = msg
